...
app.get('/api/messages/channel/:channelId/', authenticateToken, async (req, res) => {
  try {
    ...
    const channelMessages = data.filter(
...
app.get('/api/messages/dm/:userId', authenticateToken, async (req, res) => {
    try {
      ...
      const dmMessages = data.filter(m => 
        m.isDM && (
...
app.get('/api/channels', authenticateToken, async (req, res) => {
  try {
    // console.log('Fetching channels for user:', req,req.user.id);
...
app.post('/api/toggleReaction', authenticateToken, async (req, res) => {
  try {
    ...
    const messageIndex = messageId-1;//data.findIndex(m => m.id === parseInt(messageId));
    ...
    if (!data[messageIndex].reactions) {
      data[messageIndex].reactions = {};
    }
    
    // Initialize emoji array if it doesn't exist
    if (!data[messageIndex].reactions[emoji]) {
      data[messageIndex].reactions[emoji] = [];
    }

    const userReactions = data[messageIndex].reactions[emoji];
    ...
    if (userIndex === -1) {
      ...
      if (userReactions.length === 0) {
        delete data[messageIndex].reactions[emoji];
      }
    }

    await writeJSON(MESSAGES_FILE, data);
    res.json(data[messageIndex]);
  } catch (error) {
...
// Add this function to migrate existing messages
...
app.post('/api/threadMessages', authenticateToken, async (req, res) => {
  try {
    const { parentId, text, userId } = req.body;
    console.log('threadmessagespost', parentId, text, userId)
    if (!parentId || !text) {
      return res.status(400).json({ error: 'Parent ID and text are required' });
    }

    // Read threads.json
    const threadsData = await readJSON(THREADS_FILE);

    // Initialize thread array if it doesn't exist
    if (!threadsData[parentId]) {
      threadsData[parentId] = [];
    }

    // Create a new thread message
    const newThreadMessage = {
      id: threadsData[parentId].length + 1,
    ...
    await writeJSON(THREADS_FILE, threadsData);

    res.status(201).json(threadsData[parentId]);
  } catch (error) {
    console.error('Error creating thread message:', error);
    res.status(500).json({ error: 'Error creating thread message' });
  }
});

app.get('/api/threadMessages/:parentId', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    // Read threads.json
    const threadsData = await readJSON(THREADS_FILE);
    const threadMessages = threadsData[parentId];
...
app.post('/api/searchMessages', authenticateToken, async (req, res) => {
    try {
        ...
        const matchingMessages = messages.filter(message => 
          regex.test(message.text.toLowerCase())
        );
